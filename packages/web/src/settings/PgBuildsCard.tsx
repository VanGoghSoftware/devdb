...
function BuildRow(a: { row: PgBuildDto; activeMinor: number | null }) {
  ...
  const deleteDisabled = row.active || row.inUse;
  ...
  return (
    <Group justify="space-between" wrap="nowrap">
      ...
      <Group gap="xs">
        ...
        <Tooltip label={row.active ? "the active build can't be deleted" : "in use by a running endpoint"} disabled={!deleteDisabled}>
          <Button
            size="compact-xs"
            color="red"
            variant="light"

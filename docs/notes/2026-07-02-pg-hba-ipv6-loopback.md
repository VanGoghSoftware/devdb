# Compute pg_hba template: `::1/32` is not IPv6 loopback
...
DevDB's compute pg_hba assembly is oracle-derived from official `neondatabase/neon`'s
`compute_tools/src/spec.rs::update_pg_hba` (appends `params.rs::PG_HBA_ALL_MD5`, one `md5`
catch-all line, onto `initdb`'s own generated `pg_hba.conf` — neon does not otherwise rewrite the
file). The IPv6 loopback trust row is **not neon's own code** — it is stock PostgreSQL `initdb`
boilerplate, present verbatim in every fresh cluster (`pg_hba.conf.sample`, shipped by Postgres
core), which neon's compute simply inherits untouched:
...
- When implementing Task 10, do **not** "fix back" to match the oracle — the `/128` is a deliberate deviation.
- Upstream (`neondatabase/neon`, and transitively every Postgres-core-derived `initdb` default) has not been notified, per project policy of no upstream reports. If the template is ever re-synced from upstream, re-check this line.

...
> - **~~Corrected defaults: `pgRegistryBase` → `https://ghcr.io`; `pgImageTemplate` → `vangoghsoftware/worktreedb-compute-v{major}`.~~ SUPERSEDED by P2-A2 (Jordan 2026-07-09):** the DEFAULT **stays Docker Hub** (`https://registry-1.docker.io` + `neondatabase/compute-node-v{major}`) — flipping it to the PRIVATE GHCR would make a token-less `devdb:dev` 401 on every Pull (worse out-of-box than today's anonymous v17). GHCR (all-bookworm, all majors) is **opt-in** via the three env vars (`DEVDB_PG_REGISTRY_BASE=https://ghcr.io` + `DEVDB_PG_IMAGE_TEMPLATE=vangoghsoftware/worktreedb-compute-v{major}` + `DEVDB_PG_REGISTRY_TOKEN=<PAT>`). Jordan's own instance sets these; the token config + `oci.ts` Basic-auth arm (the opt-in machinery) ship unchanged. The corrected *template shape* (a repo path, no `ghcr.io/` host prefix) still holds for the opt-in value.

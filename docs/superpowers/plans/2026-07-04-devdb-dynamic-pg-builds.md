...
- **Oracle rule:** engine interactions cite `// oracle: neon <path-or-endpoint>` from official `neondatabase/neon`. The pageserver `pg_distrib_dir` semantic (per-major WAL-redo binary resolution) is upstream neon behavior; cite `pageserverToml`'s existing oracle comment when touching it.

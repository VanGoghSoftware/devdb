...
**Import:** create empty branch → start endpoint → stream `pg_dump -Fc <source> | pg_restore` → progress via job log channel → `importing → ready|failed` (stderr tail kept on failure). Whole-database only.

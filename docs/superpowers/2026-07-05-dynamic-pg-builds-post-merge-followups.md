...
## 6. Batch status (2026-07-05) + newly-surfaced residual
The post-merge cleanup batch (plan: `docs/superpowers/plans/2026-07-05-devdb-pg-builds-post-merge-cleanup.md`) landed **§3 FIX-6, §4 #7/#8/#9, §5 #11/#12 + the seedBaked-changed-major fold-in + T2/T14** (unit + web + integration-test-3 rework all green; both review gates run). **NOT done** (still deferred): §1 (endpoint-vs-build-lane concurrency-MODEL pass — architecturally-significant, its own design task), §2 (ENOTEMPTY interrupted-validating finalDir), §5 (HARD-1 supplier tuple nit).

**NEW residual surfaced by the batch's review gate (deferred, not a regression):** FIX-6 validates a marker only for UNCLAIMED dirs on adoption. An already-tracked downloaded row claims its path and `continue`s before `parseMarker`/`detectVersion`; the boot sweep only checks `bin/postgres` is accessible, not its version. So an **in-place** tamper of an already-adopted build's binary (same dir/marker, swapped binary) keeps the stale recorded minor and could dodge the downgrade flag — the same threat class FIX-6 closes for the fresh-dir vector. Deferred because: (a) it's pre-existing (the path-claim `continue` predates FIX-6, not introduced by it); (b) the primary vector (forged marker on a fresh dir) IS closed; (c) it requires write access to `/data` (a compromised container — outside a local dev tool's threat model); (d) the feature is unshipped so no pre-FIX-6 persisted rows exist. Fix if pursued: re-validate claimed downloaded ready rows at boot (path == `v{major}/{shortDigest(imageDigest)}` + re-`detectVersion`, fail on disagreement) — a boot-reconciliation addition worth its own review.

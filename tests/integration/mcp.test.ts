...
import { api, connect, waitForLsnResolvable, withConnection } from "./helpers/pg.js";

const connStr = (text: string) => text.match(/postgresql:\/\/\S+/)![0];

// waitForLsnResolvable (helpers/pg.ts, shared with timetravel.test.ts) is the deterministic gate
// before restore_branch: kind:"future" only flips to "present" once a LATER commit lands (never on
// wall-clock waiting alone). restore_branch's MCP handler calls TimeTravelService.lsnAtTimestamp
// internally and surfaces its DevdbError(400, "...kind=future...") as an isError tool result (see
// tools.ts's guard()), so polling the same pre-existing REST GET /api/branches/:id/lsn endpoint that
// restore itself resolves against is the identical mechanism the product already exposes — not a
// shortcut. (handover §8 point 5 / §3 point 5.)
...
describe("mcp acceptance flow (spec v1 items 3-4)", () => {
  ...
  it("branches isolate writes from main, show fork context + client, and support reset + restore-as-new-branch", async () => {
    ...
    // withConnection: reset_branch stopped+restarted agent/mutate's compute; a fresh connect+read
    // can race that restart under load — reconnect+retry on a transient drop (helpers/pg.ts).
    const resetTxt = await call("reset_branch", { project: "shop", branch: "agent/mutate" });
    await withConnection(dev, connStr(resetTxt), async (afterReset) => {
    ...
    // withConnection: the recovered branch's compute was just started; reconnect+retry on a
    // transient drop while it finishes coming up under load (helpers/pg.ts).
    await withConnection(dev, connStr(restoreTxt), async (recovered) => {

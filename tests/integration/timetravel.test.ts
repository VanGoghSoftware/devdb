...
describe("time travel", () => {
  ...
  it("restores dropped data in place and via new branch", async () => {
    ...
    // withConnection: the recovered branch's compute was just started; a fresh connect+read can
    // race its final startup under load — reconnect+retry on a transient drop (see helpers/pg.ts).
    await withConnection(dev, rbEp.connectionString, async (rc) => {
    ...
    // withConnection: in-place restore stopped+restarted main's compute (swapOntoNewTimeline); the
    // read can race that restart under load — reconnect+retry on a transient drop (helpers/pg.ts).
    await withConnection(dev, rEp.connectionString, async (rc) => {
      expect((await rc.query("SELECT v FROM t")).rows).toEqual([{ v: "precious" }]);
    });
  });
  ...
  it("reset returns a branch to its parent's state", async () => {
    ...
    const bc = await connect(dev, brEp.connectionString);
    ...
    // withConnection: reset stopped+restarted scratch's compute; reconnect+retry on a transient
    // drop while it finishes coming back up under load (helpers/pg.ts).
    await withConnection(dev, rEp.connectionString, async (rc) => {
      expect((await rc.query("SELECT count(*)::int AS n FROM base")).rows[0].n).toBe(1);
    });
  });
});
